...
```
/sync-gdoc <prd-name>                       # 直接同步
/sync-gdoc <prd-name> --dry-run             # 只列改動 plan，不實際寫入 GDoc
/sync-gdoc                                   # 沒指定 PRD 時掃描詢問
```

如果沒有指定 PRD，檢查 `prds/` 目錄下的 active PRDs，詢問 PM 要同步哪份。

### `--dry-run` 模式

不實際呼叫任何 GDoc 寫入 API（不 batchUpdate、不 addDocumentTab、不寫 frontmatter），只輸出**會發生什麼**：

- 哪些分頁會被建立（新檔案）
- 哪些分頁會被覆蓋（既有檔案 — 列出舊內容前 N 行 vs 新內容前 N 行）
- 哪些分頁會保留（不再存在的檔案）
- 預估 batchUpdate request 數量
- 預估執行時間

PM review 後若 OK，再跑沒有 `--dry-run` 的版本實際同步。
...
1. 列出將執行的所有動作（不實際呼叫 API）：

   ```
   📋 sync-gdoc dry-run plan — {prd-name}

   Target Doc: [新建 / 既有 {gdoc_url}]
   ...
   Tabs to overwrite:
     - Release Notes (preview before/after)
       Before (first 5 lines):
         ...
       After (first 5 lines):
         ...

   Tabs preserved (file removed locally):
     - HC Content (last synced 2026-04-12)

   Estimated batchUpdate requests: ~85
   Estimated time: ~12 sec
   ```

2. 提醒 PM：「dry-run 結束。實際同步請拿掉 `--dry-run` 重跑。」**不**繼續 Step 2b 之後的步驟。

### Step 2b：建立或準備 Google Doc
