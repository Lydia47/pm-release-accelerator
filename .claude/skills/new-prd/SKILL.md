---
name: new-prd
description: "建立新的 PRD draft。從 product thinking 評估結論或 PM 描述出發，讀取現有 Product Spec，產生結構化的 PRD 初稿。Triggers on: new prd, 新需求, 新功能, create prd."
---
...
- 從對話上下文中自動擷取已討論過的結論（包含 `/product-thinking` 的結果），不重複詢問

## Workflow

### Step 1：收集資訊

從對話上下文中整理已知資訊（product-thinking 評估結論、先前的討論等），只補問缺少的：
...
- 問題定義與用戶痛點
- 評估過的解法方向與取捨
- `/product-thinking` 的結果（ICE 評分、風險、指標、假設，如有）
- 最終選定的方向與理由
...
- **Success Metrics** — 從 product-thinking 帶入，或留空讓 PM 補充
